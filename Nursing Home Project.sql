...
WITH cte AS (
  SELECT
    ...
    SUM(Residents_Weekly_Confirmed_COVID_19 - Residents_Weekly_All_Deaths) AS TotalRecoveries
  FROM (
     SELECT Provider_State, Week_Ending, Residents_Weekly_Confirmed_COVID_19, Residents_Weekly_All_Deaths
    FROM nursing_homes_2020
    UNION ALL
    SELECT Provider_State, Week_Ending, Residents_Weekly_Confirmed_COVID_19, Residents_Weekly_All_Deaths
    FROM nursing_homes_2021
...
---3. Determine the top 7 States with the Highest COVID-19 Mortality Rate (Deaths Per Case) in 2022
WITH cte AS (
  SELECT
    Provider_State AS State,
    SUM(Residents_Weekly_Confirmed_COVID_19) AS TotalCases,
...
--2020
SELECT Provider_Name, ROUND(AVG(Residents_Weekly_Confirmed_COVID_19),2) AS average_daily_cases
FROM nursing_homes_2020
...
---7.  Determine the Top 15 Nursing Homes with the Highest COVID-19 Mortality Rates(Deaths Per COVID-19 Case) 
...
WHERE Residents_Weekly_Confirmed_COVID_19 IS NOT NULL)
SELECT Provider_Name, ROUND(MAX(moving_average),0) AS Highest_Peak
FROM cte
GROUP BY Provider_Name
...
SELECT
  ...
  CASE
    WHEN SUM(Total_Cases) > 0 THEN SUM(Total_Deaths) * 1.0 / SUM(Total_Cases)
    ELSE 0
...
---10. Determine the Top 20 Providers with the Lowest Vaccination Rates in 2023 
